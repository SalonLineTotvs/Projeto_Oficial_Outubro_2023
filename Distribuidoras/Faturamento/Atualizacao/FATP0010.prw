...
DEFINE MSDIALOG oWindow FROM 38,16 TO 600,1000 TITLE Alltrim(OemToAnsi("..MONTA ONDA POR FILIAL"+" - ULTIMA ATUALIZAÇÃO DO ESTOQUE "+ALLTRIM(GETMV("ES_FATP011")))) Pixel
...
cQuery +=	" FROM SC5020 C5 WITH (NOLOCK) "
	// IF alltrim(xfilial)=='1201' // WINTER $ GETMV("ES_FATP12R")
	// 	cQuery +=	" INNER JOIN SC6020 C6 WITH (NOLOCK) ON C5_FILIAL+C5_NUM = C6_FILIAL+C6_NUM AND C6.D_E_L_E_T_='' AND C6_LOCAL <> '95' " 
	// else
...
cQryPgr +=	" INNER JOIN SC6020 C6 WITH (NOLOCK) ON C5_FILIAL+C5_NUM = C6_FILIAL+C6_NUM AND C6.D_E_L_E_T_='' AND C6_LOCAL <> '95' " 
	...
	If MsgYesNo("DESEJA PROCESSAR OS PEDIDOS SELECIONADOS?")
		...
		For nx := 1 To Len(aBrwPV)
			...
			If cAntec == "SIM"
				...
				If !aBrwPV[nx][1]                         /// Paramtros da Procedure (1=Filial 2=Pedido 3=(Antec.) 4=Usuario 5=Voume Granel 6 = Onda)
					cExecProc := " exec uspSC6020CaixaGranel '"+aBrwPV[nx][2]+"','"+aBrwPV[nx][3]+"','"+cTpPed+"','"+cNomUser+"','"+Transform(aBrwPV[nx][11],"99999")+"','"+cNRonda+"' "
					TcSqlExec(cExecProc)
				Endif
		...
		Processa({|| U_WB_LOG001() },"Aguarde, integrando pedidos para logística...")
...
/*
		cQuery1 :=	" SELECT C5_FILIAL, C5_NUM, C5_CLIENTE, C5_LOJACLI, C5_X_STAPV, C5_NOTA 	" 
		cQuery1 +=	" FROM SC5020 C5 (NOLOCK)					"
		cQuery1 +=	" WHERE C5.D_E_L_E_T_ = ''					"
		cQuery1 +=	" AND C5_X_STAPV = '0' 						"
		cQuery1 +=	" AND C5_NOTA = ''							"
		cQuery1 +=	" AND C5_CLIENTE IN ("+cCliSel+")			"	
		cQuery1 +=	" AND C5_ESPECI4 <> 'S'						"
		cQuery1 +=	" ORDER BY C5_NUM							"
		
		TCQUERY cQuery1 NEW ALIAS "TRBP10A"

		If Select("TRBP10A") > 0
			TRBP10A->( dbCloseArea() )
		EndIf

		DBSelectArea("TRBP10A")
		TRBP10A->(dbGoTop())

		While TRBP10A->(!EOF())

	        aAdd(aPV, {ALLTRIM(TRBP10A->C5_FILIAL), ALLTRIM(TRBP10A->C5_NUM), ALLTRIM(TRBP10A->C5_CLIENTE ) })
		...
		If Len(aPV) > 0
			LogPV()
			MsgAlert("Existem outros pedidos do mesmo cliente, favor acessar a Pasta C:\TEMP\Pedidos_Pendentes.txt","Atenção")
		Endif
*/
...
//		Alert("Remover o Alerta e Return")
//		Return()
...
cQryC5 +=	" GROUP BY F4_DUPLIC,C5_FILIAL, C5_NUM, C5_CLIENTE, C5_LOJACLI, A1_NOME, A4_NREDUZ, A3_NREDUZ, C5_X_ANTEC, C5_VOLUME2, C5_X_TLNGR "
cQryC5 +=	" ORDER BY 1,2 "
...
oListBox3	:= twBrowse():New(059,005,586,215,,{" ","FILIAL","PEDIDO","CLIENTE","LOJA","NOME CLIENTE","NOME TRANSP.","VENDEDOR","GRANEL","VOL.GRANEL","VALOR TOTAL","TIPO PEDIDO"},,oTelVis,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
