			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					...
					//Utiliazando o PE Padrão: PE01NFESEFAZ
					/**********************************************
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
							...
							@aTotICMSST,len(aProd), nX, @nValDifer, cIndPres, lExpCDL, @aMonof02, @aMonof15, @lMonof53, @lMonof61 )
	Next nX
...
cEan		:= IIF(!Empty(aProd[03]),aProd[03], aProd[46])
...
cEantrib	:= IIF(!Empty(aProd[03]),aProd[03], iif( aProd[08] <> aProd[11],"",cEan))
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "RJ"
					...
					elseif cArt274 == "1"
					//comentado SALONLINE 27/09/2023... bloco abaixo , pois ja é tratado no C5_MENPAD o preenchimento da msg
...
cString += '<infadprod>'+AllTrim(ConvType(aProd[25],500)+cMensDeson+cDedIcm+cCrgTrib+cMensFecp+cMsgMonofa+' '+aProd[52])+'</infadprod>'
