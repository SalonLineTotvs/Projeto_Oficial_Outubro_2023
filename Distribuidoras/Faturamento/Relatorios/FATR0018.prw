...
cQueryC += " Z1_PLACA,Z1_PALLETS,Z1_VEICULO,F2_DOC, F2_EST, "
...
cQueryC += "    , sum(PFECHADO) PFECHADO"

cQueryC += " FROM SF2020 SF2 "
cQueryC += " INNER JOIN SZ1020 SZ1 ON SZ1.D_E_L_E_T_=''  AND Z1_FILIAL=F2_FILIAL AND Z1_NRMA=F2_X_NRMA " 
...
cQueryC += " ON SA4.D_E_L_E_T_=''  AND A4_COD = CASE WHEN Z1_TRANSP=' ' THEN F2_TRANSP ELSE Z1_TRANSP END"

cQueryC += " INNER JOIN ("
cQueryC += " SELECT C6_FILIAL, C6_NUM, C6_PRODUTO, SUM(PFECHADO) PFECHADO FROM("
cQueryC += " SELECT C6_FILIAL, C6_NUM, C6_PRODUTO, "
cQueryC += " CASE WHEN B1_XQTDPAL>0 THEN FLOOR((C6_QTDVEN-C6_X_VGRIM )/B1_XQTDPAL) ELSE 0 END PFECHADO"
cQueryC += " FROM SC6020 C6  (NOLOCK)"
cQueryC += " INNER JOIN SB1020 B1 ON C6_PRODUTO = B1_COD AND B1_FILIAL = '' AND B1.D_E_L_E_T_ = ''"
cQueryC += " WHERE C6.D_E_L_E_T_=' ' AND C6_NOTA<>' '"
cQueryC += " )TOTSC6 GROUP BY C6_FILIAL, C6_NUM, C6_PRODUTO" 
cQueryC += " )PEDIDO ON D2_FILIAL=C6_FILIAL AND D2_PEDIDO=C6_NUM AND D2_COD=C6_PRODUTO"

//cQueryC += " WHERE F2_EMISSAO BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+ "' "
cQueryC += " WHERE Z1_DTMA BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+ "' "
...
TcSetField("TSQLM","PFECHADO"	,"N",3,0)
...
IF !TSQLM->(eof())
	...
	oExcel:AddColumn(cNomAba		,cNomTit	,"PALLETS FECHADO"	,1		,1		,.F.	)
	...
	Do While TSQLM->(!Eof())
		...
		TSQLM->F2_EMISSAO,TSQLM->A1_NOME,TSQLM->F2_EST,TSQLM->A4_NOME,;
		TSQLM->QTD,TSQLM->VLR_MERC,TSQLM->F2_VOLUME1,TSQLM->F2_PLIQUI,TSQLM->F2_PBRUTO,TSQLM->PFECHADO})
		//							 	   
		TSQLM->(DbSkip())
		//
	Enddo     
