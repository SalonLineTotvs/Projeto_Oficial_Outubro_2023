...
IF (Upper(GetEnvServer()) $ AllTrim(GetMV('SN_CLSLOBO',.F.,'LOBO'))) .And. GetMV('SN_CLSTRUE',.F.,.F.)
	u_SN_CLSLOGO('zCpySX1','zCpySX1.prw') //1º User Function | 2° Nome.PRW
ENDIF //__________________________________________________________________________________________________
	...
	Else
		...
		EndIf
	EndIf
	MsgInfo("Arquivo de parametros copiado com sucesso"+ENTER+ENTER+'Pergunta antiga: '+cPergAnt+ENTER+'Pergunta nova: '+cPergNov, "Aviso")
	RestArea(aAreaX1)
	RestArea(aArea)
Return