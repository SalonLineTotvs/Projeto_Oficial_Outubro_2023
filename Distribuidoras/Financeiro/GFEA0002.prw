...
// Solicita��o - Marcio / Didacio / Pedro
// Autor       - Andr� Salgado - 20/04/2022
//****************************************************

USER FUNCTION XGFEA02()
...
@ 247,182	To 350,550 Dialog oNota Title OemToAnsi("PARAMETRO PROCESSAR A FATURA NO ...")
@ 003,003	Say OemToAnsi("SELECIONAR A NATUREZA ?")
@ 003,075	Get _cNaturez valid !empty(_cNaturez) PICTURE "@!" F3 "SED" size 50,50 
...
@ 039,003	Say OemToAnsi("Data Vencimento Boleto ?")
@ 039,075	Get _dVencto valid !empty(_dVencto) size 50,50

@ 045,075   BmpButton Type 1 Action Close(oNota)
Activate Dialog oNota Centered
Set Device To Print


    if msgyesno("Confirma a Integracao dos CTE desta Fatura Nr."+cTituFAT+" ?")

        //Corre��o das Faturas com erro na Entrada do Lan�amento - Inclui "0" a esquerda
        _cUpGXI := " UPDATE GXI020 SET GXI_NRFAT=RIGHT('0000000000'+RTRIM(GXI_NRFAT),10)"
        _cUpGXI += " WHERE D_E_L_E_T_=' ' AND SUBSTRING(GXI_NRFAT,10,1)=' '"
        TcSqlExec(_cUpGXI)
        ...
        cQuery += " ,XML_CHAVE"
        cQuery += " ,CASE WHEN RIGHT(RTRIM(XML_MUNMT),2)=RIGHT(RTRIM(XML_MUNDT),2) THEN '153' ELSE '153' END CFOP"
        ...
        cQuery += " RIGHT('000000000'+RTRIM(GXJ_NRDF),9) = RIGHT(RTRIM(XML_NUMNF),9) "
        cQuery += "      AND XML_EMISSA between GXG_DTEMIS-10 and GXG_DTEMIS+10"
        ...
        MemoWrit("C:\aa\valigfe_"+cTituFAT+".TXT",cQuery)
        ...
        //Valida dados se est�o validados
        cFornVaz := ""

        //Valida se Esta Logado na Empresa correta
        if TRBOP->GXI_FILIAL <> cFilAnt
            Alert("Fatura pertence a Empresa - "+TRBOP->GXI_FILIAL+", para importar selecionar a Distribuidora correta.  Processo ser� abortado !"+" Foi digitado nos parametros:"+;
        ...
        if !empty(cFornVaz)
            Alert("CTE nao tem fornecedor cadastro, operacao abortada ate o cadastro. CTE Nr:"+cFornVaz)
            return
        Endif


        //APRESENTA A INFORMA�AO PARA USUARIO ANTES DE PROCESSAR
        _cMens := "Esta integracao pode levantar alguns minutos, pode iniciar ?"+CRLF


        //ULTIMA CONFIRMACAO ANTES DA GRAVACAO
        if msgyesno(upper(_cMens))
            ...
            While TRBOP->(!EOF())
                ...
                ElseIF _cNaturez $ "9005A"
                    cCodProd:= "SV0000000000068"    //DEVOLU��O
                ...
                if nValUni>0
                    ...
                    //Salva dados para Informar o Usuario no final da Importa��o
...
//Grava Fatura
    ...
    MakeDir( "C:\Temp\" )
    cMsg := "Importacao concluida - Valor CTE Importado - "+transform(nTotCte,"@E 9999,999.99")+_cEnter+cNrCte+_cEnter
    if !empty(cMsgPG)
       cMsg += cMsgPG+_cEnter
    Endif
    cMsg += "Importado por - "+Alltrim(UsrRetName(__CUSERID))+" em "+dtoc(date())
    MemoWrit("C:\Temp\ImpCTE_Fatura_"+cTituFAT+".TXT",cMsg)
...
if empty(TRBOP->A2_COD)
    Alert("Fatura nao tem fornecedor cadastro, operacao abortada ate o cadastro. Fatura Nr:"+TRBOP->GXI_NRFAT)
    return
Endif
...
//Valida se Esta Logado na Empresa correta
if TRBOP->GXI_FILIAL <> cfilant
    Alert("Fatura pertence a Empresa - "+TRBOP->GXI_FILIAL+", para importar selecionar a Distribuidora correta.  Processo sera abortado !")
    return
Endif
...
//        aAdd(aAuto,{"E2_VENCREA" , DataValida(stod(TRBOP->GXI_DTVENC),.T.)  ,Nil})				
        aAdd(aAuto,{"E2_VENCTO"  , _dVencto             ,Nil})				
        aAdd(aAuto,{"E2_VENCREA" , DataValida(_dVencto,.T.)  ,Nil})				
