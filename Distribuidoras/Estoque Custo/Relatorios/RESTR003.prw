...
cQuery  += " 				AND SD1.D1_DOC = '"+MV_PAR07+"' "
cQuery  += " 				AND SD1.D1_SERIE = '"+MV_PAR09+"' "
cQuery  += " 				AND SD1.D1_FORNECE = '"+MV_PAR05+"' "
cQuery  += " 				AND SD1.D1_LOJA = '"+MV_PAR11+"' "
...
cQuery  += "ORDER BY SD1.D1_DOC+SD1.D1_COD+SD1.D1_LOTECTL+SD1.D1_DTVALID " 
*/
memowrite('c:\temp\restr003.txt',cQuery) 
...
while !TRBSD1->(EOF())
 ...
 //	If cTravamento <> SD1->(D1_DOC+D1_COD+D1_LOTECTL+DTOS(D1_DTVALID) )
	    ...
	    NNR->(DbSeek(xFilial("SD1") + SD1->D1_LOCAL))
 		aDesc := U_Trata1(SB1->B1_DESC,40)
 		
 		nValTotDes := SD1->D1_TOTAL - SD1->D1_VALDESC
 		nValUnit := nValTotDes / SD1->D1_QUANT 

 		
		oPrn:Say( nLin + 05, 0050, SD1->D1_COD,  ArialN10B)   
...
If nLin < 3200 //3400 
	...
	oPrn:Say( nLin + 360, 0050, "TOTAL quaren R$ "+Str(nTotquaren) ,  ArialN10B) 
...
oPrn:Say( nLin + 150, 0400, "("+SD1->(D1_FORNECE+D1_LOJA)+") "+Substr(SA1->A1_NOME,1,45), ArialN12B, 100 )
...
User Function Trata1(cMsg, nTotCarac)
...
/* esta rotina fica em sf1140.prw
User Function XMLCTE07()
Local aUsrBtn := ParamIxb[1]
aadd(aUsrBtn,{"Preto",{|| u_Getate(),Eval(bRefrPerg) },"Gera Call Center"}) 
aadd(aUsrBtn,{"Preto",{|| u_restr003(),Eval(bRefrPerg) },"Relatorio Devolucao"}) 
