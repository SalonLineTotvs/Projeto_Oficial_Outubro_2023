...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
   ...
   31/12/2019  VALMIR         Sol.Valdir - Quando Imprimir DANFE será flag o campo SF2->F2_X_IMPRI="1" (1)
...
Local cAmbiente := GetEnvServer() //SALONLINE GENILSON
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								...
								//*** SALONLINE - (1) - Solic. Valdir - Quando imprimir DANFE Flag como Danfe Impressa
								//*** COPIAR VARIÁVEL cAmbiente
								If !(upper(cAmbiente) $ ('SCHEDULE'))
									SF2->F2_X_IMPRI	:= "1"	//Campo Customizado.
								EndIf
					...
					ElseIf lIsLoja
						...
						If !Empty(aXML[nX][9]) .AND. Val(aXML[nX][9]) > 200
							...
							Replace SF2->F2_X_IMPRI	with "1"  //Campo Customizado.
							SF2->( MsUnlock() )
