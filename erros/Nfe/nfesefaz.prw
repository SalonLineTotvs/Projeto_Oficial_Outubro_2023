// VERSAO - NFSEFAZ 30-04-2020 - release 25
...
//							If !Empty(SC6->C6_NUMPCOM) .And. !Empty(SC6->C6_ITEMPC) 	//LINHA ORIGINAL
						...
						If SM0->M0_ESTENT $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
							...
							//*** VALMIR (09/04/20 - ATENDER CHAMADO DIDACIO - TK2004032 ERRO 930 - kOPEK) - SALONLINE
							If SM0->M0_ESTENT = "RJ"
								If cCST == "20"
									cCodlan :="RJ802164"
								Endif
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							(nDescIcm+nDescRed+nDescNfDup),;	//(3) SALON LINE - Remover nDesconto e Adicionado nDescZF
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				// *********  MELHORIAS SALON LINE
				//IIf(Val(SB1->B1_X_EAN13)==0,"",StrZero(Val(SB1->B1_X_EAN13),Len(Alltrim(SB1->B1_X_EAN13)),0)),;	//(1) Salonline
				//IIF(!((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO $ "D" .And. cTpOrig == "P")) ,(IIF(!(lMvNFLeiZF),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL - ((cAliasSD1)->D1_DESCZFP+(cAliasSD1)->D1_DESCZFC))),0),; //(2)
				//nDescRed + nDescIcm + nDescNfDup + (cAliasSD1)->D1_VALDESC + nDescFis -(SFT->FT_DESCZFR),; //(3)
				//IIF(!((cAliasSD1)->D1_TIPO $"IP"),(cAliasSD1)->D1_VUNIT,0),; (4)
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_TOTAL-(cAliasSD1)->D1_VALDESC,0),;		//(2) SalonLine +nDescZF
					...
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
					nDescRed + nDescIcm + nDescNfDup +  nDescFis ,; //(3) Salonline
					IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_VUNIT-( (cAliasSD1)->D1_VALDESC / (cAliasSD1)->D1_QUANT) ,0),;					//(4) SalonLine
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
	           ...
	           //Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devolução de compra com IPI não tributado apenas para saida
				...
				Else				
					...
					-(IIF(Alltrim(SF4->F4_MOTICMS)  $ "7",nDescZF,0	))  				//(01) SALONLINE ADICIONADO 
				EndIf
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
			    ...
			    If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
					...
					// SALONLINE - MENSAGEM COMENTADA DEVIDO SOLICITAÇÃO DO VALDIR
					/*
