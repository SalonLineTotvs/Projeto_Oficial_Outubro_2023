// VERSAO - NFSEFAZ 10-05-2019 - NOVA LEI 
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
    16/03/18 ³ Andre Salgado ³Incluir a Mensagem art 313 conforme TES     
    16/03/18 ³ Andre Salgado ³Incluir Descricao Produto Base ST e Vlr ST 
    16/03/18 ³ Andre Salgado ³Melhorar informações do ARRAY (Saida / Entrada)
               Genilson       Incluir Mensagem do Cliente  cMensCli += ALLTRIM(SA1->A1_X_MSGNF)
			   Genilson 	  Incluir Mensagem Nr PEDIDO DANFE
			   Genilson 	  Alterar Regra padrao do Pedido de Compra
			   Genilson 	  Incluir regra codigo SUFRAMA para cliente isento do Beneficio
			   Genilson 	  Comentar linha XML > cString += '<nItemPed>'+ConvType(aPedCom[02])+'</nItemPed>'
...
// SALONLINE
// ALTERADO B1_CODBAR para B1_X_EAN13
...
Local nVlrStAnt := nImpStAnt := 0 //*** SalonLine  16/03/18
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					...
					If !Empty(SA1->A1_X_MSGNF)
						cMensCli += ALLTRIM(SA1->A1_X_MSGNF)
					EndIf
					
					If !Empty(cMensCli)
						cMensCli += " "
					EndIf
					cMensCli += "NOS PAGAMENTOS APOS O VENCIMENTO SERAO COBRADOS ENCARGOS MORATORIOS | "
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_BRICMS,				//*** SalonLine No Padrão não tem Base ST - André Salgado (16/03/18)								
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If !(AllTrim(SC5->C5_NUM) $ cMensCli)
							cMensCli += "PEDIDO: " + SC5->C5_NUM
						EndIf
						If  !(AllTrim(SC6->C6_NUMPCOM) $ cMensCli) .and. !Empty(SC6->C6_NUMPCOM)
							cMensCli += " | REF CLIENTE: " + SC6->C6_NUMPCOM		
						EndIf
						...
						If SF4->F4_ART274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
							If !("Tags vBCSTRet e vICMSSTRet do XML" $ cMensFis)
								cMensFis += "Valores referentes a Base de Cálculo ICMS/ST e valores Retidos são os constantes nas Tags vBCSTRet e vICMSSTRet do XML. "
							EndIf
						EndIF	
						...
						// SALONLINE - GENILSON LUCAS - RETIRADO ITEM SOMENTE XPED
//						If SC6->(FieldPos("C6_NUMPCOM")) > 0 .And. SC6->(FieldPos("C6_ITEMPC")) > 0    //LINHA ORIGINAL
						If SC6->(FieldPos("C6_NUMPCOM")) > 0 //.And. SC6->(FieldPos("C6_ITEMPC")) > 0  //SALONLINE - ALTERADA
//							If !Empty(SC6->C6_NUMPCOM) .And. !Empty(SC6->C6_ITEMPC) 	//LINHA ORIGINAL
							If !Empty(SC6->C6_NUMPCOM) //.And. !Empty(SC6->C6_ITEMPC) 	//SALONINE - ALTERADA
					    ...
					    //SALONLINE - (André Salgado) - Data 16/03/2018
						//Tratamento para Descrição + Base de Calculo ST e Valor ST do ART 313
						//cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI) 		//linha original
						IF SF4->F4_ANTICMS="1" .and. SA1->A1_USADDA="1"		//!!! Importante foi utilizado este campo pra gerar msg só p/Determinados clientes
							cDescProd := AllTrim(IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI))
							IF (cAliasSD2)->D2_BRICMS>0 .AND. (cAliasSD2)->D2_ICMSRET>0
								cDescProd += " - Base ST:"+Transform((cAliasSD2)->D2_BRICMS,"@E 999999.99")
								cDescProd += "  Valor ST:"+Transform((cAliasSD2)->D2_ICMSRET,"@E 99999.99")								
							Endif
						Else
						...
						//SALONLINE	 - André Salgado  - Alteração ARRAY Dados (SAIDA)
						//IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;  //(1) Linha Original
						//IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),; //(2) Linha Original
						//(nDesconto+nDescIcm+nDescRed),;		//(3) Linha Original
						aadd(aProd,	{Len(aProd)+1,;
							cCodProd,;
							IIf(Val(SB1->B1_X_EAN13)==0,"",StrZero(Val(SB1->B1_X_EAN13),Len(Alltrim(SB1->B1_X_EAN13)),0)),;	//(1) SALON LINE
							cDescProd,;
							...
							SB1->B1_UM,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;	//(02) SALONLNIE
							...
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							(nDescIcm+nDescRed+nDescNfDup+nDescZF),;	//(3) SALON LINE - Remover nDesconto e Adicionado nDescZF
							...
							SB1->B1_CODBAR,; // SALONLINE - CAMPO DO DUN14
							})
				...
				// Msg Zona Franca de Manaus / ALC
					...
					If !SF2->F2_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )//Desnecessario seek redundante na SF3 pois o campo F2_DESCZFR ja possui os valores de ZFR de toda a venda
						...
						ElseIF !lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0) 
					    	...
					    	Else
						    	cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT,13,2)+ " SUFRAMA: " + aDest[15]
						EndIF

				    	//SALONLINE - adiconado código do suframa (Genilson M Lucas)
				    	Else													
						cMensFis += "Código Suframa: "+alltrim(aDest[15]+".")	
					//***	Fim da Melhoria		***
					EndIf 	
			    ...
			    //SALONLINE - (André Salgado) - Data 16/03/2018
				//Tratamento para mensagem em informacoes adicionais do ART 313
				IF SF4->F4_ANTICMS="1"
					dbSelectArea("SF3")
					dbSetOrder(4)
					dbSeek (xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
					Do While !SF3->(Eof()) .AND. xFilial("SF3") == SF3->F3_FILIAL .And.;
						...
						SF2->F2_DOC == SF3->F3_NFISCAL .And. SF2->F2_SERIE == SF3->F3_SERIE
		  				...
		  				nVlrStAnt += SF3->F3_BASERET
		  				nImpStAnt += SF3->F3_ICMSRET
						SF3->(DbSkip ())
	   				EndDo		
	   				
	   				//SALONLINE - COMENTADO MSG
	   				//cMensFis += "Imposto Recolhido por Substituição - Contempla os artigos 273, 313 do RICMS. Valor da Base de ST: R$ "+str(nVlrStAnt,15,2)+" Valor de ICMS ST: R$ "+str(nImpStAnt,15,2)+" "
				EndIF
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					IIf(Val(SB1->B1_X_EAN13)==0,"",StrZero(Val(SB1->B1_X_EAN13),Len(Alltrim(SB1->B1_X_EAN13)),0)),;	//(1) Salonline
					cDescProd,;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_TOTAL-(cAliasSD1)->D1_VALDESC,0),;		//(2) SalonLine
					...
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
					(nDescRed + nDescIcm) + (IIF((cAliasSD1)->D1_TIPO $ "D" .AND. nDescZF > 0,(cAliasSD1)->D1_DESC,0 )),; //(3) Salonline
				   	IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_VUNIT-(cAliasSD1)->D1_VALDESC,0),;					//(4) SalonLine
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
...
Local nValDeson		:= 0
Local nEmbal		:= 0 //SALONLINE - QTD EMBALAGEM DO PRODUTO
...
// SALONLINE // CUSTOMIZAÇÃO PARA ATENDER GRUPO GPA *** GENILSON LUCAS
If '47508411' $ alltrim(aDest[01])
	cString += '<ean>'+aProd[49]+'</ean>'
Else
...
// SALONLINE // CUSTOMIZAÇÃO PARA ATENDER GRUPO GPA *** GENILSON LUCAS
If '47508411' $ alltrim(aDest[01]) 
	nEmbal	:= Posicione("SB1",1,xFilial("SB1")+aProd[02],"B1_QE")
	
	cString += '<uCom>'+'CX'+'</uCom>'
	cString += '<qCom>'+ConvType(aProd[09] / nEmbal ,15,4) +'</qCom>'
	cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType( (aProd[10]/aProd[09]) * nEmbal ,21,8))+'</vUnCom>'
Else
